...
func (t *RequestTransformer) OpenAIResponseToAnthropic(body []byte) ([]byte, error) {
	...
	// Anthropic content must be an array of content blocks.
	text, _ := t.extractOpenAIContent(openAIResp).(string)
	content := []interface{}{
	...
	// usage is optional on some backends; guard the assertion.
